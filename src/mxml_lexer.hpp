...
#ifndef MXML_LEXER_HPP_
#define MXML_LEXER_HPP_
...
namespace __mxml {
	...
	typedef class _mxml_lexer {
		...
		public:
			...
			_mxml_lexer(void);
			_mxml_lexer(const std::string &input, bool is_file);
			_mxml_lexer(const _mxml_lexer &other);
			virtual ~_mxml_lexer(void);
			_mxml_lexer &operator=(const _mxml_lexer &other);
			mxml_buffer &get_buffer(void);
